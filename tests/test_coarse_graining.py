...
import pytest
import igraph as ig
from src.coarse_graining import coarse_grain_strenghts, coarse_grain_weighted_graph

@pytest.mark.parametrize(['strenghts', 'group_sequence', 'expected'],
                [([[1,1], [2,1], [1,3]], [[0,1], [2]], [[3,2], [1,3]]),
                 ([[1,1], [2,1], [1,3]], [[0], [1], [2]], [[1,1], [2,1], [1,3]]),
                 ([[1,1], [2,2], [3,3]], [[0,1,2]], [[6,6]])]
)
def test_coarse_grain_strenghts(strenghts, group_sequence, expected):
    assert (coarse_grain_strenghts(strenghts, group_sequence) == expected).all()
